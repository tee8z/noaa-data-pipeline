...
impl FileAccess {
    ...
    pub fn build_file_paths(&self, file_names: Vec<String>) -> Vec<String> {
        file_names
            .iter()
            .map(|file_name| {
    ...
    //Body::from_stream
    pub async fn grab_file_names(&self, params: FileParams) -> Result<Vec<String>, Error> {
    ...
    pub fn build_file_path(&self, filename: &str, file_generated_at: OffsetDateTime) -> String {
        format!(
            "{}/{}/{}",
            self.data_dir,
            file_generated_at.date(),
